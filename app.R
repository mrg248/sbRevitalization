...
# Any code not marked for a team member was done collaboratively

# Kevin Gallagher overall code organization
...
ui <- fluidPage(
  ...
  mainPanel( 
    tabsetPanel( 
      tabPanel("Restaurants & Abandoned Properties", # Kevin Gallagher
               leafletOutput("combinedMap", height = "800px", width = "100%")),
      
      tabPanel("Redevelopment Corridors",  # Nicole McAlee
               fluidPage(
      ...
      tabPanel("Demographic Insights",  # Carolina SantaMaria
               sidebarLayout(
                 sidebarPanel(
                   selectInput("mapType",
                               "Select Map Type:",
      ...
      tabPanel("Income by City Council District",  # Nicole McAlee
               fluidPage(
...
server <- function(input, output, session) {
  ...
  output$combinedMap <- renderLeaflet({
    ...
    # Create map  
    # Final Editing by Carolina SantaMaria
