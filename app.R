...
# Ensure both datasets are in the same CRS and are 'sf' objects
...
# Selecting relevant columns, removing NA values (census tracts outside the city boundaries), and sorting
...
ui <- fluidPage(
  ...
  mainPanel(
    tabsetPanel(
      ...
      # New Tab for Income by City Council District
...
server <- function(input, output, session) {
  ...
  output$combinedMap <- renderLeaflet({
    # Assuming 'License_Ex' is the field indicating the date of license expiration
    restaurantData <- businessData %>%
      filter(Classifi_1 %in% c("RESTAURANTS A-M", "RESTAURANTS N-Z")) %>%
